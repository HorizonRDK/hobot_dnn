...
class Tuple {
 public:
   ...
   */
  inline int32_t NDim() const { return static_cast<int32_t>(ndim_); }
...
class TShape : public Tuple<uint32_t> {
 public:
   ...
   */
  inline uint32_t ProdSize() const {
    return ProdSize(0U, static_cast<uint32_t>(NDim()));
  }
  ...
  */
  inline uint32_t ProdSize(uint32_t begin) const {
    return ProdSize(begin, static_cast<uint32_t>(NDim()));
  }
