...
#if defined(HR_MAC) && !defined(WEBRTC_IOS)
...
enum LoggingSeverity {
  LS_SENSITIVE,
  LS_VERBOSE,
  LS_DEBUG, // added by minghao
  LS_INFO,
  LS_WARNING,
  LS_ERROR,
  LS_FATAL, // added by minghao
  LS_NONE,
  INFO = LS_INFO,
  WARNING = LS_WARNING,
  LERROR = LS_ERROR,

  // hobot log severity alias
  HOBOT_LOG_VERBOSE = LS_VERBOSE,
  HOBOT_LOG_DEBUG = LS_DEBUG,
  HOBOT_LOG_INFO = LS_INFO,
  HOBOT_LOG_WARN = LS_WARNING,
  HOBOT_LOG_ERROR = LS_ERROR,
  HOBOT_LOG_FATAL = LS_FATAL,
  HOBOT_LOG_NULL = LS_NONE,
};
...
class LogMessage {
 ...
 private:
  ...
  LogMessage(const LogMessage &rhs) = delete;
