...
class DnnExampleNode : public DnnNode {
 ...
 private:
  int DnnParserInit();
  void SetYolov2OutParser(Model* model_manager);
  void SetYolov3OutParser(Model* model_manager);
  void SetYolov5OutParser(Model* model_manager);
  void SetFasterRcnnOutParser(Model* model_manager);
  int Render(cv::Mat &mat, Perception &perception);

  std::string config_file = "";
  int32_t output_index_ = 2;
  dnnParsers parser = dnnParsers::KPS_PARSER;
