...
#ifndef FCOS_DET_NODE_H_
#define FCOS_DET_NODE_H_
...
struct FcosOutput : public DnnNodeOutput {
  std::string image_name = "";
  std::shared_ptr<std_msgs::msg::Header> image_msg_header = nullptr;
};

class FcosDetNode : public DnnNode {
 public:
  FcosDetNode(const std::string &node_name,
              const NodeOptions &options = NodeOptions());
  ~FcosDetNode() override;
 ...
 private:
  ...
  int Render(cv::Mat &mat, const FcosDetResult *);
 ...
 private:
  ...
  // 模型相关参数
  std::string model_file_name_ = "config/fcos_512x512_nv12.bin";
  ...
  const int32_t fcos_output_index_ = 14;
